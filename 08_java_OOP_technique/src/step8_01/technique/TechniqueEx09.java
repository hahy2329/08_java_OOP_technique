...
//추상클래스(책 목차 제목,지도 등) - abstract 키워드를 클래스 앞에 붙여준다.
abstract class Parent2{
	
	
	
	//목차 - abstract 키워드를 메서드 앞에 붙여준다.(몸체를 만들지 않는다.)
	abstract void test1();
	abstract void test2(); 
	abstract void test3();
	abstract void test4();
	...
	int testVar; //추상 클래스에서는 일반 멤버변수도 같이 사용이 가능하다.
	void test99() { // 추상 클래스에서는 메서드도 같이 사용이 가능하다.
